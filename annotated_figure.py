...
def plot_single_file_sigma_only(
    ...
    ax.text(-1.3,-0.5,'Planet', color='blue')
    ax.set_xlim(-4,4)
    ax.set_ylim(-4,4)
...
def raise_figure_windows(plot_fn, figsize=[3.32, 4]):
    #print(filename)
    fig = plt.figure(figsize=figsize)
    plot_fn(fig, filename, **get_ranges())
    #fig.suptitle(filename)
    plt.subplots_adjust(left=0.155,bottom=0.08,right=0.93,top=0.92,wspace=0.2,hspace=0.2)
    plt.show()
...
if __name__ == "__main__":
    ...
    manual_fit = plt.Circle((-0.86,-0.86),2.8, color='blue', fc='None')
    inner_edge = plt.Circle((0,0),1.5, color='red', fc='None')
    outer_edge = plt.Circle((0,0),3.0, color='red', fc='None')
    plot_annulus()
